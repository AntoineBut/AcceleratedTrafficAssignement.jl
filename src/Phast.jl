...
function backward!(node_order::Vector{Int}, g_down::G, weights::Dict{Tuple{Int,Int},Float64}, source::Int, distances::Vector{Float64}) where G <: AbstractGraph
	...
	for node in 1:nv(g_down)
		for u in inneighbors(g_down, node)
			edge_weight = weights[(u, node)]
...
function shortest_path_CH2(g_CH::CHGraph, source::Int)
	# Computes the shortest paths from source to all other nodes using the Contraction Hierarchy.
	# It uses a bidirectional Dijkstra search on the upward and downward graphs.
	g_up = g_CH.g_up
	g_down_rev = g_CH.g_down_rev
	...
	node_order = g_CH.node_order
	distances = fill(Inf, nv(g_CH.g))
	forward2!(g_up, weights, source, distances)
	backward2!(node_order, g_down_rev, weights, source, distances)

	return distances

end

function forward2!(g_up::G, weights::Dict{Tuple{Int,Int},Float64}, source::Int, distances::Vector{Float64}) where G <: AbstractGraph
	# Performs a forward search on the upward graph from the source node.
	# Returns the shortest distances from source to all reachable nodes in g_up.
...
function backward2!(node_order::Vector{Int}, g_down_rev::G, weights::Dict{Tuple{Int,Int},Float64}, source::Int, distances::Vector{Float64}) where G <: AbstractGraph
	# Iterates through nodes in reverse rank order.
	# For each node, recompute the shortest distance from incoming edges : d[v] = min(d[v], d[u] + w(u,v))
