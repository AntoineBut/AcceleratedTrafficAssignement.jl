...
function dfs!(graph::G, source::Int, visited::Vector{Bool}, order::Vector{Int}, index::Int) where G <: AbstractGraph
	stack = Stack{Int}()
	push!(stack, source)
	while !isempty(stack)
		node = pop!(stack)
...
function permuted_graph(order::Vector{Int}, graph::G, old_weights::Dict{Tuple{Int,Int},Float64}) where G <: AbstractGraph
	n = nv(graph)
	graph_permuted = SimpleDiGraph(n)
